...
class TelegramAlert:
    ...
    def send_weekly_signal_with_context(
        ...
        if ema_200:
            above_label = "Above \u2713" if above_ema else "Below \u2717"
            ema_line = f"\U0001f4ca EMA 200: ${ema_200:,.0f} ({above_label})\n"
        else:
            ema_line = ""

        boost_label = "Boost \u2713" if signal_data.get('mvrv_boost_eligible') else "No Boost"
        ...
        message = (
            ...
            f"{ema_line}"
            f"\U0001f4c8 MVRV: {signal_data.get('mvrv', 0):.2f} ({boost_label})\n"
